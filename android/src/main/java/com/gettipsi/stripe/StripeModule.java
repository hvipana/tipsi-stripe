...
public class StripeModule extends ReactContextBaseJavaModule {
  ...
  public void createTokenWithBankAccount(final ReadableMap accountData, final Promise promise) {
    try {
      ...
      mStripe.createBankAccountToken(
        new BankAccountTokenParams(accountData.getString("country"), accountData.getString("currency"), accountData.getString("accountNumber")),
        mPublicKey,
        null,
        new ApiResultCallback<Token>() {
          public void onSuccess(Token token) {
  ...
  private PaymentMethodCreateParams extractPaymentMethodCreateParams(final ReadableMap options) {
    ...
    } else if(sepaDebitParams != null) {
      sepa = new PaymentMethodCreateParams.SepaDebit.Builder().
        setIban(sepaDebitParams.getString("iban")).
        build();
      return PaymentMethodCreateParams.create(
        sepa,
        billingDetails,
